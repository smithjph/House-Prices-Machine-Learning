...
test_x <- data.frame(scale(test_x))



# train svm using e1071 package
model_e1071 <- svm(y_train ~ ., train_x, scale = FALSE)

predicted_e1071 <- predict(model_e1071, test_x)

# predict and write to CSV file
submission = fread(SUBMISSION, colClasses = c("integer","numeric"))
submission$SalePrice = predicted_e1071
submission$SalePrice = exp(submission$SalePrice)
write.csv(submission,"SVM_e1071.csv", row.names = FALSE)




# train svm using caret package
model_svmLinear <- train(train_x, y_train, method = 'svmLinear2')

# predict and write to CSV file
submission2 = fread(SUBMISSION, colClasses = c("integer","numeric"))
submission2$SalePrice = predict(model_svmLinear, test_x)
submission2$SalePrice = exp(submission2$SalePrice)
write.csv(submission2,"SVM_caret.csv", row.names = FALSE)
