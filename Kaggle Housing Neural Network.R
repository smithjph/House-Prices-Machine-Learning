...
net1 <- train(train_x, y_train, method = 'nnet', linout = 1)

# predict and write to CSV file
submission = fread(SUBMISSION, colClasses = c("integer","numeric"))
submission$SalePrice = predict(net1, test_x)
submission$SalePrice = exp(submission$SalePrice)
write.csv(submission,"NetCaretModel.csv", row.names = FALSE)
...
# train neural network
net2 <- nnet(train_x, y_train, size = 10, linout = TRUE)

# predict and write to CSV file
submission = fread(SUBMISSION, colClasses = c("integer","numeric"))
submission$SalePrice = predict(net2, test_x)
submission$SalePrice = exp(submission$SalePrice)
write.csv(submission,"nNetModel.csv", row.names = FALSE)
